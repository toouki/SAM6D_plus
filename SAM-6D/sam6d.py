...
# from Instance_Segmentation_Model.utils.data_utils import rle_to_binary_mask as rle_to_mask
...
class PoseEstimationDetector:
    ...
    def __init__(self, segmentor_model="sam", 
                pem_checkpoint=None,
                cam_path=None, 
                cad_path=None, 
                output_dir=None, 
        ...
        Args:
            ...
            init_models_immediately: 是否立即初始化模型（默认False，保持原有行为）
        """
        ...
        if self.cad_path:
            ...
            self.model_points = mesh.sample(2048).astype(np.float32) / 1000.0  # 转为米单位
        
        logger.info(f"初始化SAM6D检测器，分割模型: {segmentor_model}, 设备: {self.device}")
        
        if not self.output_dir:
            raise ValueError("启用立即初始化时必须提供output_dir参数")
    ...
    def _init_pose_templates(self):
        ...
        try:
            # 加载模板数据
            self.all_tem, self.all_tem_pts, self.all_tem_choose = self.get_templates(
    ...
    def get_templates(self, path, cfg):
        """获取所有模板"""
        ...
        for v in range(n_template_view):
            ...
            tem, tem_choose, tem_pts = self._get_template(path, cfg, i)
            all_tem.append(torch.FloatTensor(tem).unsqueeze(0).cuda())
            all_tem_choose.append(torch.IntTensor(tem_choose).long().unsqueeze(0).cuda())
            all_tem_pts.append(torch.FloatTensor(tem_pts).unsqueeze(0).cuda())
        return all_tem, all_tem_pts, all_tem_choose

    def get_det_data(self, rgb_path, depth_path, cam_path, cad_path, seg_path, det_score_thresh, cfg):
        """获取测试数据"""
        dets = []
        # 加载分割结果JSON文件
    ...
    def _load_or_compute_descriptors(self, output_dir, cfg, reset_descriptors, cad_path=None):
        """加载或计算模板描述符"""
    ...
    def run_instance_segmentation(self, rgb_path, depth_path):
        ...
        # 检查必要参数
        if not self.cam_path or not self.cad_path or not self.output_dir:
            raise ValueError("初始化时必须提供cam_path, cad_path, output_dir参数")
        ...
        # 处理深度和相机数据，准备几何分数计算
        batch = batch_input_data(depth_path, self.cam_path, self.device)
        # 获取模板的姿态分布(用于投影计算)
    ...
    def run_pose_estimation(self, rgb_path, depth_path, seg_path, det_score_thresh=0.2):
        ...
        # 加载分割数据
        input_data, img, whole_pts, model_points, detections = self.get_det_data(
        ...
        if ninstance == 0:
            ...
            return [], img, model_points * 1000  # 返回空结果，但保持正确的返回格式
        
        # 检查是否已预加载模板
        if self.all_tem_pts is None or self.all_tem_feat is None:
            logger.warning("模板未预加载，正在临时加载...")
            self._init_pose_templates()
            
        if self.all_tem_pts is None or self.all_tem_feat is None:
            logger.error("模板加载失败，无法进行姿态估计")
            return [], img, model_points * 1000
        ...
        # 应用姿态NMS
        # 使用默认值
        rot_thresh = getattr(self.pem_config, 'rot_thresh', 3.0)  # 默认3度
        trans_thresh = getattr(self.pem_config, 'trans_thresh', 5.0)  # 默认5mm
    ...
    def detect(self, rgb_path, depth_path):
        """
        完整的6D物体姿态检测流程
        ...
        # 步骤1: 实例分割
        detections, query_appe_descriptors, best_template, pred_idx_objects, ref_aux_descriptor = self.run_instance_segmentation(rgb_path, depth_path)
        ...
        # 保存ISM分割结果
        detections.to_numpy()
        ism_save_path = f"{self.output_dir}/sam6d_results/detection_ism"
    ...
    def _visualize_results(self, pose_results, img, model_points, output_dir):
        ...
        for idx, result in enumerate(pose_results):
            # 可视化单个实例
            save_path = os.path.join(output_dir, f"vis_pose_instance_{idx}_score_{result['score']:.4f}.png")
            
            # 获取相机内参（从输入数据中获取）
            # 注意：model_points需要转为毫米单位，与参考实现保持一致
            vis_img = visualize_single(
                img, 
            ...
            logger.info(f"已保存实例 {idx} 的可视化结果: {save_path}")
